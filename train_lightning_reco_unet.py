...
from utilities.DS import MRIRecoDataset, SpaceNormalization
from utilities.transforms import ShiftCut, ToTensor
...
class FBPConvNet(LightningModule):
    def __init__(self, **kwargs):
        ...
        self.radon = Radon(kwargs['imsize'], -fully_angles[thetaSortInd], circle=False, scikit=True)
        self.net = Net()
        if self.hparams.loss == LossType.MSE:
            ...
            alpha = .02/(.02+.001)
            self.loss = RestorationLoss(alpha=alpha, data_range=kwargs['gtMax_img'])
        self.accuracy = RMSELoss()
        self.trafo_aug = []  # [AddNoise()]
        self.trafo = [ShiftCut(imsize=kwargs['imsize'], PerformOnUnder=False, circle=False), ToTensor()]  # [SortSino(), ToTensor()]
        self.residual = self.hparams.residual
        self.example_input_array = [torch.empty(8, 1, kwargs['imsize'], kwargs['imsize'])]
...
if __name__ == '__main__':
    ...
    from test_lightning_reco_unet import main as TestMain

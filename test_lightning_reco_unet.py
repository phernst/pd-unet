...
from train_lightning_reco_unet import FBPConvNet
from utilities.DS import MRIRecoDataset
    ...
    with wandb.init(name=run_name, id=run_name, project="SinoUp", group="MRI", entity='ovgufindke', resume=True) as WnBRun:
        ...
        for _, batch in enumerate(tqdm(dataloader_test)):
            with torch.no_grad():
                ...
                sparse_img = sparse_img.cpu().numpy()
                sparse_img[sparse_img < 0] = 0

                for i in range(len(gt)):
                    path4output = pjoin(out_dir, subname[i], filename[i])
                    metrics.append(ValidateNStore(prediction[i].squeeze(), gt[i].squeeze(), sparse_img[i].squeeze(), path4output))

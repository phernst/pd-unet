...
from models.primal_dual import PrimalDualNetworkSino as Net
...
class PrimalDual(LightningModule):
    def __init__(self, **kwargs):
        ...
        self.net = Net(radon, radon_end, 4, 5, 2,
    ...
    def training_step(self, batch, _):
        ...
        full_sino = self.net.radon_end.forward(fulldata).transpose(-1, -2)
        prediction_sino, _ = self(sparse_sino, under_reco)
        loss = self.loss(prediction_sino, full_sino)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        sparse_sino = self.net.radon.forward(fulldata).transpose(-1, -2)
        full_sino = self.net.radon_end.forward(fulldata).transpose(-1, -2)
        prediction_sino, prediction_reco = self(sparse_sino, under_reco)
        loss = self.loss(prediction_sino, full_sino)
        accuracy = self.accuracy(prediction_sino, full_sino)
        ...
        if batch_idx % 10 == 0:
            ...
            reco = prediction_reco.cpu().numpy()[0][0]
            reco[reco < 0] = 0
            cv2.imwrite(
                pjoin(self.hparams.valid_dir, self.hparams.run_name,
...
def main():
    ...
    subtype: str = 'sparse'
    sparsity: int = 4
    run_name: str = f'{subtype}{sparsity}_pd_unet_sino'
    num_epochs: int = 151
    use_amp: bool = True
